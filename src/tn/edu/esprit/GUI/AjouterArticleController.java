...
public class AjouterArticleController implements Initializable {
  Connection cnx = DataS.getInstance().getConnection();
    ...
    private void EnrgArticl(ActionEvent event) throws IOException {
        ...
        if(x == 0){
        Article a = new Article(NomArt.getText(), MarqArt.getText(), TypeArt.getText(), "disponible", CouleurArt.getValue().toString(),Float.valueOf(PrixArt.getText()), TailleArt.getText(), DiscArt.getText(), FicheArtc.getText(), MagasainSel.getValue().getIdMagasin(),CatSelect.getValue().getIdCategorie());
        sa.ajouter(a);
